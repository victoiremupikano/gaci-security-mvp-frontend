...
export default function Index() {
  const [postComent, setPostComent] = useState<Array<any>>();
  const [next, setNext] = useState('');
  const [previous, setPrevious] = useState('');
  const [count, setCount] = useState()
  const [loading, setLoading] = useState(false)
  useVerify();
  const getPostComent = async (postId: string, entreprizeId: string) => {
    setLoading(true)
      const result = await PostComent.getByPost(postId, entreprizeId);
      if (result.results) {
        setPostComent(result.results);
      ...
      <div className="w-11/12 mx-auto h-full">
        <div className="w-full mb-3 mt-4 flex justify-between">
          <div className="flex items-center">
            <GoBack />{" "}
            <span className="font-semibold text-xl">Tous les commentaires isncrit</span>
          </div>
        </div>
        <div className="flex justify-between border-b text-sm p-1 font-semibold">
          <span className="w-fit">#</span>
          <span className="w-2/12 text-center">Commentaire</span>
        ...
        {!loading ? (
          Array.isArray(postComent) && postComent.length > 0 ? (
            postComent.map((a, index) => {
              return (
                <div
                  key={a.pk}
                ...
                >
                  ...
                  <span className="md:w-2/12 text-center">{a.message.substr(0,16)}...</span>
                  ...
                  <span className="md:w-1/12 w-2/12 flex justify-around">
                    <Link
                      href={"/staff/coment/" + post as string + "/" + a.pk}
