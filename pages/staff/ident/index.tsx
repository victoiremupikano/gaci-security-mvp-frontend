...
export default function Index() {
  const [idents, setIdents] = useState<Array<any>>();
  ...
  const getPas = async () => {
    setLoading(true);
    const result = await Ident.getAll();
    if (result.results) {
      setIdents(result.results);
  ...
  return (
    <>
      <ReusableHeader text="Toutes tentatives de reconnaissnce" />
      <div className="w-11/12 mx-auto h-full">
        <div className="w-full mb-3 mt-4 flex justify-between">
          <div className="flex items-center">
            <GoBack />{" "}
            <span className="font-semibold text-xl">Toutes tentatives de reconnaissnce</span>
          </div>
        ...
        <div className="flex justify-between border-b text-sm p-1 font-semibold">
          ...
          <span className="w-2/12 text-center">Noms poster</span>
        ...
        {!loading ? (
          Array.isArray(idents) && idents.length > 0 ? (
            idents.map((a, index) => {
              return (
                <div
                  key={a.pk}
