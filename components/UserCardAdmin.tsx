    ...
    >
      ...
      <div className=" w-[calc(100%-80px)] text-gray-700 p-1  flex-col justify-between flex h-[90%]">
        <div className="flex flex-col">
          <span className="flex items-center"><UserIcon className="mr-1 h-5 w-5 "/>{user.names}</span>
          <span className="flex items-center"><EnvelopeIcon className="mr-1 h-5 w-5 "/>{user.email}</span>
          <span className={` ${user.is_active  ? "text-green-600":"text-red-600"} flex items-center`}>{user.is_active ? "Actif(ve)" : "Inactif(ve)"}</span>
        </div>
